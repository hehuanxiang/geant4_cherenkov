#!/usr/bin/env python3
"""
Cherenkov Photon Analysis - Full Data Version
Clean architecture with complete dataset analysis (43.8M records)

Usage:
  python analyze_cherenkov.py              # Generate all 15 plots
  python analyze_cherenkov.py 2             # Generate only plot 2
  python analyze_cherenkov.py 5 10 14       # Generate plots 5, 10, 14
"""
...
def plot_09_displacement_vs_energy(data):
    ...
    # Sample for performance (200k points max for scatter plot)
    sample_size = min(200000, len(data['energy']))
...
def plot_10_position_correlation(data):
    ...
    # Sample for performance (50k points max for scatter plot)
    sample_size = min(50000, len(data['init_x']))
    sample_idx = np.random.choice(len(data['init_x']), sample_size, replace=False)
    
    fig, axes = plt.subplots(1, 2, figsize=(14, 6))
    
    axes[0].scatter(data['init_x'][sample_idx], data['final_x'][sample_idx], 
                   alpha=0.3, s=10, color='steelblue', label='Photons')
    ...
    axes[1].scatter(data['init_y'][sample_idx], data['final_y'][sample_idx], 
                   alpha=0.3, s=10, color='coral', label='Photons')
...
def plot_13_spatial_clustering(data):
    ...
    print(f"      Performing DBSCAN on {sample_size:,} samples...")
...
def plot_14_joint_distributions(data):
    ...
    sample_size = min(500000, len(data['energy']))
    sample_idx = np.random.choice(len(data['energy']), sample_size, replace=False)
    
    fig, axes = plt.subplots(2, 3, figsize=(16, 10))

    axes[0, 0].hexbin(data['energy'][sample_idx], data['displacement'][sample_idx], 
    ...
    axes[0, 1].hexbin(data['energy'][sample_idx], data['angle_change'][sample_idx], 
    ...
    axes[0, 2].hexbin(data['displacement'][sample_idx], data['angle_change'][sample_idx], 
                     gridsize=40, cmap='PuBu', mincnt=1)
    ...
    axes[1, 0].hexbin(data['init_z'][sample_idx], data['displacement'][sample_idx], 
                     gridsize=40, cmap='RdPu', mincnt=1)
    ...
    axes[1, 1].hexbin(data['init_z'][sample_idx], data['energy'][sample_idx], 
                     gridsize=40, cmap='GnBu', mincnt=1)
    ...
    axes[1, 2].hexbin(data['init_z'][sample_idx], data['angle_change'][sample_idx], 
                     gridsize=40, cmap='OrRd', mincnt=1)
...
def plot_15_statistical_summary(data):
    ...
    sample_size = min(100000, len(data['energy']))
    sample_idx = np.random.choice(len(data['energy']), sample_size, replace=False)
...
def main():
    ...
    # Print header
    print("=" * 70)
    print("Cherenkov Photon Analysis - Full Dataset Version")
    print("=" * 70)
