#!/usr/bin/env python3
"""
Test v2 PHSP read path in analyze_cherenkov.load_and_process_data.
"""
...
def test_analyze_cherenkov_v2_load():
    ...
    data["event_id"] = np.arange(n, dtype=np.uint32) % 5
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # Patch analyze_cherenkov's BINARY_FILE and load
        sys.path.insert(0, _project_root())
        import analyze_cherenkov as ac
        ac.BINARY_FILE = phsp_path

        result = ac.load_and_process_data()
        assert "event_id" in result
        assert "track_id" in result
        assert result["event_id"].min() >= 0
        assert result["event_id"].max() < 5
        assert result["track_id"].min() >= 1
        assert result["track_id"].max() <= n


if __name__ == "__main__":
    test_analyze_cherenkov_v2_load()
    print("test_analyze_cherenkov: OK")
